...
import 'package:friflex_dev_plugins_console/console/icon.dart' as icon;
import 'package:friflex_dev_plugins/util/floating_widget.dart';
import 'package:friflex_dev_plugins/util/store_mixin.dart';
import 'package:friflex_dev_plugins_console/console/show_date_time_style.dart';
...
class ConsoleState extends State<Console>
  ...
  List<(DateTime, String)> _logList = <(DateTime, String)>[];
      ...
      if (mounted) {
        if (_filterExp != null) {
          _logList = ConsoleManager.logData.where((e) {
            return _filterExp!.hasMatch(e.$1.toString()) ||
                _filterExp!.hasMatch(e.$2);
          }).toList();
        } else {
          _logList = ConsoleManager.logData.toList();
        }
  ...
  String _dateTimeString(int logIndex) {
    ...
    switch (_showDateTimeStyle) {
      case ShowDateTimeStyle.datetime:
        result =
            '${_logList[_logList.length - logIndex - 1].$1.toString().padRight(26, '0')}';
        break;
      ...
      case ShowDateTimeStyle.timestamp:
        result =
            '${_logList[_logList.length - logIndex - 1].$1.millisecondsSinceEpoch}';
        break;
  ...
  Future<void> _share() async {
    ...
    final l = _logList.map((e) => '${e.$1.toString()} ${e.$2}').toList();
